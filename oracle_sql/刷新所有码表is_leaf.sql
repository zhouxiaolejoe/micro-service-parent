...
declare
  ...
  DICD_CURSOR MY_CURSOR_TYPE;
BEGIN
    V_SQL:='SELECT ELE_CODE,ELE_ID,IS_LEAF,PARENT_ID  FROM ELE_VD09001';
    OPEN DICD_CURSOR FOR V_SQL;
    LOOP
    FETCH DICD_CURSOR INTO ELE_CODE,ELE_ID,IS_LEAF,PARENT_ID;
    EXIT WHEN DICD_CURSOR%NOTFOUND;    
    EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM ELE_VD09001 WHERE IS_DELETED = 2  AND FISCAL_YEAR =2022 AND PARENT_ID = '''|| ELE_ID ||'''' INTO K;
    IF K < 1 THEN
      EXECUTE IMMEDIATE 'UPDATE ELE_VD09001 SET IS_LEAF = 1 WHERE FISCAL_YEAR =2022 AND ELE_ID = '''|| ELE_ID ||'''';
    ELSE
       EXECUTE IMMEDIATE 'UPDATE ELE_VD09001 SET IS_LEAF = 2 WHERE FISCAL_YEAR =2022 AND ELE_ID = '''|| ELE_ID ||'''';
    END IF;
    COMMIT;   
    END LOOP; 
END;
