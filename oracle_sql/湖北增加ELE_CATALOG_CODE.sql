  ...
  for t_row in t_tables loop
   ...
   IF H < 1 THEN
     execute immediate '
      ...
      values ('''||sys_guid()||''', ''' || t_row.ele_source || ''', ''BE00001'', ''目录编码'', ''ELECATALOGCODE'', ''String'', ''38'', ''oracle'', ''i3FPlwwB'', null, ''1'', ''1'', ''1'', to_date(to_char(sysdate, ''yyyyMMddHH24:mm:ss''),''yyyy/MM/dd HH24:mi:ss''), to_date(to_char(sysdate, ''yyyyMMddHH24:mm:ss''),''yyyy/MM/dd HH24:mi:ss''), to_date(to_char(sysdate, ''yyyyMMdd''), ''yyyy/MM/dd''), ''2021'', ''109900000'', null, ''0'', ''00001'', ''varchar2'', ''1'', ''1'', ''0'', ''ELE_CATALOG_CODE'')
     ';
     commit;
   END IF;
  end loop;
